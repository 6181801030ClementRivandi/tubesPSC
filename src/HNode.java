...
public class HNode extends Node implements Comparable<HNode> {
	...
	private int f; //variabel untuk menampung hasil dari fungsi A* yang berisi panjang edges ditambah dengan heuristiknya
	
	public HNode(Node node, int h) {
		super(node.getState(), node.getDepth(), node.getParent());
		this.f = node.getDepth() + h; //memasukkan nilai akhir ke variabel f
